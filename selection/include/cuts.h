...
{   
    ...
    */
    ...
    /**
     * @brief Cut to remove events with more than one non-primary EM shower.
     * @details This function counts non-primary particles identified as photons 
     * or electrons (PID <= 1) with a kinetic energy above a threshold.
     * @tparam T the type of interaction (true or reco).
     * @param obj the interaction to select on.
     * @param params params[0] sets the KE threshold. Defaults to 25 MeV.
     * @return true if the interaction has at most one non-primary EM shower.
     */
    template<class T>
    bool single_shower(const T & obj, std::vector<double> params={25.0,})
    {
        ...
        for(const auto & p : obj.particles)
        {
            ...
            if(count > 1) 
                return false; // Break early to save computation
        }
        return true;
    }
